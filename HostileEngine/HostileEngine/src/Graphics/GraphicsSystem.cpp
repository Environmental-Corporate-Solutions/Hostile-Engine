...
namespace Hostile
{
    ...
    InstanceData GraphicsSys::ConstructInstance(const std::string _mesh, const std::string _material, const UINT32 _id)
    {
        ...
        instance.id = IGraphics::Get().CreateInstance(instance.meshId, instance.materialId, _id);
        return instance;
    }
    ...
    void GraphicsSys::OnCreate(flecs::world& _world)
    {
        ...
        m_renderTargets.push_back(IGraphics::Get().CreateRenderTarget(1));
        m_readback_buffers.push_back(IGraphics::Get().CreateReadBackBuffer(m_renderTargets[1]));
    ...
    void GraphicsSys::PostUpdate(flecs::iter const& _info)
    {
        ...
        ImVec2 stuff = ImGui::GetContentRegionAvail();
        ImGui::Image(
        ...
        if (ImGui::IsWindowFocused() && ImGui::IsWindowDocked())
        {
            ...
            if (ImGui::IsItemClicked())
            {
                    ...
                    && mouse_pos.x >= min.x && mouse_pos.y >= min.y)
                {
                    ...
                    Vector2 pos = {
                        (mouse_pos.x - min.x) * vp.x / ImGui::GetItemRectSize().x,
                        (mouse_pos.y - min.y) * vp.y / ImGui::GetItemRectSize().y
                    };
                    Log::Debug(std::string("Mouse Pos: ") + "(" + std::to_string(pos.x) + ", " + std::to_string(pos.y) + ")");
                }
            }
        }
